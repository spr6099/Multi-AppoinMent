...
const MyAppointments = () => {
    ...
    const pendingCount = appointments.filter(
        item => item.isCompleted === false && item.cancelled === false
    ).length;
    ...
    return (
        <div>
            <div className="flex gap-10">
                ...
                <p>Cancelled Appointments: {cancelledCount}</p>
                ...
                {appointments.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        ...
                        <div className='flex flex-col gap-2 justify-end text-sm text-center'>
                            {!item.cancelled && !item.payment && !item.isCompleted && payment !== item._id && <button onClick={() => setPayment(item._id)} className='bg-blue-500 text-white sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300'>Pay Online</button>}
                            ...
                            {!item.cancelled && !item.isCompleted && <button onClick={() => cancelAppointment(item._id)} className='bg-red-400 text-[white] sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300'>Cancel appointment</button>}
